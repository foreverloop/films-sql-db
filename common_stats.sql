...
TO 'csv/film_category_by_count.csv' (format CSV);
...
COPY(SELECT COUNT(film.film_id),actor.first_name,actor.last_name FROM actor 
	...
	ORDER BY COUNT(film.film_id) DESC) 
TO 'csv/most_common_actors.csv' (format CSV);
...
TO 'csv/most_common_cities.csv' (format CSV);
...
TO 'csv/category_counts.csv' (format CSV);

--find the most profitable film type (also include how many of each type of film)
COPY(SELECT category.name, SUM(COALESCE(payment.amount,0)) category_value,COUNT(film.title)
FROM payment 
...
INNER JOIN category ON film_category.category_id = category.category_id 
GROUP BY category.name ORDER BY SUM(COALESCE(payment.amount,0)) DESC,COUNT(film.title) ASC)
TO 'csv/most_profitable.csv' (format CSV);
